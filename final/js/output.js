...
var titleScreen = function titleScreen(game) {};
...
playGame.prototype = {
    ...
    deathReset: function deathReset() {
        if (player.y > 16 * 60) {
            player.reset();
            this.music.stop();
    ...
    stillAnimation: function stillAnimation() {
        if (player.body.velocity.y < 0) {
            player.animations.play("jump");
        } else if (player.body.velocity.y > 0) {
            player.animations.play("fall");
    ...
    createTileMap: function createTileMap(levelNum) {
        if (levelNum == 1) {
            levelNum = "";
        }
    ...
    setTopCollisionTiles: function setTopCollisionTiles(tileIndex, layer) {
        var x, y, tile;
            ...
            for (y = 1; y < globalMap.height; y++) {
                ...
                if (tile !== null) {
                    ...
                    if (Array.isArray(tileIndex) == true) {
                        tileIndex.every(function (value, index, array) {
                            if (tile.index == value) {
    ...
    bindKeys: function bindKeys() {
        ...
        game.input.keyboard.addKeyCapture([Phaser.Keyboard.SPACEBAR, Phaser.Keyboard.LEFT, Phaser.Keyboard.RIGHT, Phaser.Keyboard.DOWN, Phaser.Keyboard.UP]);
    },
    ...
    camFollow: function camFollow(target) {
        game.camera.follow(target, Phaser.Camera.FOLLOW_PLATFORMER);
    },
    collectStar: function collectStar(player, item) {
        this.increaseScore();
        item.kill();
        this.audioStar.play();
    },
    collectCarrot: function collectCarrot(player, item) {
        item.kill();
    ...
    collectLoot: function collectLoot(player, item) {
        if (item.able) {
            item.kill();
            this.audioStar.play();
            this.increaseScore();
    ...
    killZone: function killZone(obj) {
        if (obj.kind == "player") {
            obj.death();
        }
    },
    exitZone: function exitZone(obj) {
        if (obj.kind == "player") {
            this.music.stop();
            this.game.state.start("gameOver");
        }
    },
    triggerLadder: function triggerLadder(obj) {
        if (obj.kind == "player" && this.wasd.up.isDown) {
            obj.onLadder = true;
        }
    },
    enemyCollide: function enemyCollide(obj) {
        if (obj.kind == "slug") {
            // enemy.y = 0;
            obj.turnAround();
        }
    },
    updateHealthHud: function updateHealthHud() {
    ...
    checkAgainstEnemies: function checkAgainstEnemies(player, enemy) {
    ...
    checkAgainstChests: function checkAgainstChests(player, chest) {
